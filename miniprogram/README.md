...
## 中文整本朗读与自动翻页（2026-09-07）

阅读页只保留中文播放／暂停，不提供语言选择，中英文正文仍同屏。播放使用微信 BackgroundAudioManager，整本只设置一次音频地址，按 onTimeUpdate 的真实播放位置与逐页时间轴同步页码和中文高亮；手动翻页使用 seek，暂停翻页保持暂停。锁屏或返回继续播放，再进入同书时读取 currentTime 同步页码。最后结束停止，播放错误可重试。阅读指南和放大插图会暂停。

离线制作命令：

```sh
npm run mini:prepare
npm run mini:audio:books -- --upload --workers=4
npm run mini:full
```

生成脚本只读取已导出的公开绘本，复用现有 Supabase 逐页中文 Edge TTS 缓存，缺失时生成。逐页音频解码为 24 kHz 单声道 PCM，按采样数记录时间轴，拼接后统一编码成 MP3，并重新解码检查时长偏差不超过 50 ms。整本 MP3 和同名 JSON 时间轴上传到独立公开桶 library-audio-public，不调整原 story-audio 私有桶权限。凭证从本地环境读取，不写入小程序。依赖 ffmpeg。

完整生成后会输出可版本管理的 chinese-audio-manifest.json，只含永久公开地址、时间轴和正文哈希；全新检出也能直接准备播放包。中间缓存和进度清单位于被忽略的 miniprogram/book-audio-cache，失败后重跑会复用已完成文件。--limit=1 可只准备第一本，不带 --upload 只生成本地产物。音频清单采用内容哈希校验；中文正文改变后旧成品不打包，需重跑生成。--upload 会发布仅含公开绘本的成品。生成与上传完成后再次运行 mini:full 才会刷新小程序包中的地址。

小程序直接使用永久公开音频地址，无需现场调用生成接口或打包即将过期的签名链接。尚未准备成品的书目只显示图文；full-library/report.json 的 preparedBooks 应等于 books 后才上传正式版本。本地备用音频预览仍可使用中文前台播放。

真机重点：锁屏持续播放跨越多页、解锁同步、手动跳到前后页、暂停后翻页再继续、系统进度拖动、末页停止及断网重试。预览编译与自动化测试不能替代 iPhone／Android 验收。全量测试二维码 full-library/preview-qr-chinese-all-210.png。
