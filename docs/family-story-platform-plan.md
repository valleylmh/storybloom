...
本地用户生成作品的正式成品预览已经复用馆藏核心阅读器，并与馆藏一致采用云端 TTS 优先、浏览器 `SpeechSynthesis` 兜底。匿名用户可使用公共音色并受现有 IP 限流保护；精选案例和定制案例仍保留旧 `NarrationToolbar`。页面管理、插图重试和修复区没有被阅读器替代，只在界面上与“阅读和播放”明确分区。
...
- 本地用户成品使用统一阅读器并启用云端 TTS 优先；精选/定制案例继续保留现有浏览器朗读，避免一次性改动所有预览变体。
