...
| B | 西游记连载系列 | P1 | A（复用 A 的全部基建） | 已完成 10 回并全部上线 |
...
公版 IP 连载系列，制造"追更"复访动力。每回一本 8 页书，首批 10 回。

### 任务

1. **内容规划**：从原著挑适合低龄的前期情节，首批 10 本：石猴出世、拜师学艺、龙宫借宝、大闹天宫、五行山下、师徒相遇、白龙马、高老庄遇八戒、流沙河收沙僧、三打白骨精（温和智斗版）。⚠️ 低龄改编原则写入生成 prompt：打斗改为"比试/智斗/合作解决问题"，妖怪形象趣味化不恐怖，不出现死亡描写。
...
**验收标准**：
- [x] 首批 10 回完整上线，上一回/下一回导航可用；未来新增未发布回目时仍保留「即将更新」逻辑。
- [x] 孙悟空在不同书之间外观一致（人工对比确认）。

> B 完成记录（2026-07-23）：系列扩展到 10 回、每回 8 页中英双语，共 80 张插图；全部使用 Codex 内置生图工具按系列角色卡逐页生成，输出为 1200×1200 WebP 且单张低于 300KB。新增并锁定金箍棒、玉帝、唐僧、细金色头环、小白龙/白龙马、猪八戒、沙僧及白骨精温和云灵等跨回视觉锚点；每回均有 4×2 接触表和逐页审核记录。10 回全部标记 `imageStatus: "complete"`、移除 `comingSoon` 并进入 sitemap。
